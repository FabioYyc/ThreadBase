...
export const getAccessToken = async ({authorizeCode, type, refresh_token}:{authorizeCode?: string, type: 'authorize' | 'refresh', refresh_token?:string}): Promise<any> => {
    const url = 'https://auth.atlassian.com/oauth/token';

    if(!process.env.CONFLUENCE_APP_CLIENT_ID || !process.env.CONFLUENCE_APP_SECRET || !process.env.CONFLUENCE_AUTH_URL) {
...
export const fetchCfUrl = async ({cloudId, accessToken, path, method}:{cloudId: string, accessToken: string, path:string, method: 'GET' | 'POST'}) => {
