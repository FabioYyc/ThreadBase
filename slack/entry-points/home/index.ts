...
import { getThreadsTabViewByUser } from "./home-tab-view";
...
import { saveNoteHandlers } from "./save-note/handler";
...
const homeOpenListener = (app: App) => {
  app.event("app_home_opened", async ({ event, client, body }) => {
    try {
      ...
      const view = await getThreadsTabViewByUser(orgId, event.user);
